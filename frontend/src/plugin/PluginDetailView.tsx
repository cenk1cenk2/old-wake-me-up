    ...
    return (
      <DefaultPage title={name} maxWidth={1000}>
        ...
        {capabilities.indexOf('configurer') !== -1 ? (
          <PanelWrapper name="Configurer" description="This is the configuration panel for this plugin." icon={Build} refresh={this.refreshConfigurer.bind(this)}>
            <ConfigurerPanel
              pluginInfo={pluginInfo}
              initialConfig={this.state.currentConfig !== null ? this.state.currentConfig : 'Loading...'}
        ...
        {capabilities.indexOf('displayer') !== -1 ? (
          <PanelWrapper name="Displayer" description="This is the information generated by the plugin." refresh={this.refreshDisplayer.bind(this)} icon={Subject}>
            <DisplayerPanel pluginInfo={pluginInfo} displayText={this.state.displayText !== null ? this.state.displayText : 'Loading...'} />
          </PanelWrapper>
        ) : null}
      </DefaultPage>
    )
...
const PanelWrapper: React.SFC<IPanelWrapperProps> = ({ name, description, refresh, icon, children }) => {
  const Icon = icon
  return (
    <Container
      style={{
    ...
    return (
      <div>
        ...
        <Button
          ...
          color="primary"
          fullWidth={true}
          disabled={this.state.unsavedChanges === null || this.state.unsavedChanges === this.props.initialConfig}
...
class PluginInfo extends Component<{ pluginInfo: IPlugin }> {
  public render () {
    const { props: { pluginInfo: { name, author, modulePath, website, license, capabilities, id, token } } } = this
    return (
      <div>
        {name ? (
        ...
        {capabilities.indexOf('webhooker') !== -1 ? (
          <Typography variant="body2">
            Custom Route Prefix:{' '}
            {((url) => (
              <a href={url} target="_blank" rel="noopener noreferrer" className="custom-route">
                {url}
              </a>
