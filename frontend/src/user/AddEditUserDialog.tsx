...
export default class AddEditDialog extends Component<IProps, IState> {
  ...
  public render () {
    ...
    return (
      <Dialog open={true} onClose={fClose} aria-labelledby="form-dialog-title" id="add-edit-user-dialog">
        <DialogTitle id="form-dialog-title">{isEdit ? 'Edit ' + this.props.name : 'Add a user'}</DialogTitle>
        <DialogContent>
          <TextField autoFocus margin="dense" className="name" label="Name *" type="email" value={name} onChange={this.handleChange.bind(this, 'name')} fullWidth />
          <TextField
            margin="dense"
            className="password"
            type="password"
          ...
          />
          <FormControlLabel
            control={<Switch checked={admin} className="admin-rights" onChange={this.handleChecked.bind(this, 'admin')} value="admin" />}
            label="has administrator rights"
          />
        </DialogContent>
        <DialogActions>
          <Button onClick={fClose}>Cancel</Button>
          <Tooltip placement="bottom-start" title={namePresent ? (passPresent ? '' : 'password is required') : 'name is required'}>
            <div>
              <Button className="save-create" disabled={!passPresent || !namePresent} onClick={submitAndClose} color="primary" variant="contained">
                {isEdit ? 'Save' : 'Create'}
              </Button>
            </div>
          </Tooltip>
