...
class Users extends Component<WithStyles<'wrapper'> & Stores<'userStore'>> {
  ...
  public render () {
    const { deleteId,
      editId,
      createDialog,
      props: { userStore } } = this
    const users = userStore.getItems()
    return (
      <DefaultPage
        title="Users"
        rightControl={
          <Button id="create-user" variant="contained" color="primary" onClick={() => (this.createDialog = true)}>
            Create User
          </Button>
        }
      >
            ...
            <Table id="user-table">
              ...
              <TableBody>
                {users.map((user: IUser) => {
                  return <UserRow key={user.id} name={user.name} admin={user.admin} fDelete={() => (this.deleteId = user.id)} fEdit={() => (this.editId = user.id)} />
                })}
              </TableBody>
            </Table>
          </Paper>
        </Grid>
        {createDialog && <AddEditDialog fClose={() => (this.createDialog = false)} fOnSubmit={userStore.create} />}
        {editId !== false && (
          <AddEditDialog
