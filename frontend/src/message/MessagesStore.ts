...
export class MessagesStore {
  ...
  public constructor (private readonly appStore: BaseStore<IApplication>, private readonly snack: SnackReporter) {
  ...
  public loadMore = async (appId: number) => {
    ...
    const pagedResult = await this.fetchMessages(appId, state.nextSince).then((resp) => resp.data)
  ...
  private fetchMessages = (appId: number, since: number): Promise<AxiosResponse<IPagedMessages>> => {
    if (appId === AllMessages) {
      return axios.get(config.get('url') + 'message?since=' + since)
    } else {
      return axios.get(config.get('url') + 'application/' + appId + '/message?since=' + since)
    }
  }

  private getUnCached = (appId: number): (IMessage & { image: string | null })[] => {
    const appToImage = this.appStore.getItems().reduce((all, app) => ({ ...all, [app.id]: app.image }), {})
