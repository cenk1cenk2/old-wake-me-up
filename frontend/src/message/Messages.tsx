...
class Messages extends Component<IProps & Stores<'messagesStore' | 'appStore'>, IState> {
  ...
  public componentWillMount () {
    window.onscroll = () => {
      if (window.innerHeight + window.pageYOffset >= document.body.offsetHeight - window.innerHeight * 2) {
  ...
  public render () {
    ...
    return (
      <DefaultPage
        ...
        rightControl={
          <div>
            <Button id="refresh-all" variant="contained" disabled={!hasMessages} color="primary" onClick={() => messagesStore.refreshByApp(appId)} style={{ marginRight: 5 }}>
              Refresh
            </Button>
            <Button id="delete-all" variant="contained" disabled={!hasMessages} color="primary" onClick={() => messagesStore.removeByApp(appId)}>
              Delete All
            </Button>
          </div>
        }
      >
        {hasMessages ? (
          <div style={{ width: '100%' }} id="messages">
            <ReactInfinite key={appId} useWindowAsScrollContainer preloadBatchSize={window.innerHeight * 3} elementHeight={messages.map((m) => this.heights[m.id] || 1)}>
  ...
  private deleteMessage = (message: IMessage) => () => this.props.messagesStore.removeSingle(message)
