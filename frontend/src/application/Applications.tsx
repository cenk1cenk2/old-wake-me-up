  ...
  public render () {
    ...
    return (
      <DefaultPage
        title="Applications"
        rightControl={
          <Button id="create-app" variant="contained" color="primary" onClick={() => (this.createDialog = true)}>
            Create Application
          </Button>
        }
        maxWidth={1000}
      >
        <Grid item xs={12}>
          <Paper elevation={6}>
            ...
            <input ref={(upload) => (this.upload = upload)} type="file" style={{ display: 'none' }} onChange={this.onUploadImage} />
          </Paper>
        </Grid>
        {createDialog && <AddApplicationDialog fClose={() => (this.createDialog = false)} fOnSubmit={appStore.create} />}
        {updateId !== false && (
          <UpdateDialog
            fClose={() => (this.updateId = false)}
            fOnSubmit={(name, description) => appStore.update(updateId, name, description)}
...
const Row: SFC<IRowProps> = observer(({ name, value, noDelete, description, fDelete, fUpload, image, fEdit }) => (
  <TableRow>
