...
import ToggleVisibility from '@root/components/toggle-visibility.component'
  ...
  public render () {
    ...
    return (
      <DefaultPage
        title="Clients"
        rightControl={
          <Button id="create-client" variant="contained" color="primary" onClick={() => (this.showDialog = true)}>
            Create Client
          </Button>
        }
      >
            ...
            <Table id="client-table">
              ...
              <TableBody>
                {clients.map((client: IClient) => {
                  return <Row key={client.id} name={client.name} value={client.token} fEdit={() => (this.updateId = client.id)} fDelete={() => (this.deleteId = client.id)} />
                })}
              </TableBody>
            </Table>
          </Paper>
        </Grid>
        {showDialog && <AddClientDialog fClose={() => (this.showDialog = false)} fOnSubmit={clientStore.create} />}
        {updateId !== false && (
          <UpdateDialog fClose={() => (this.updateId = false)} fOnSubmit={(name) => clientStore.update(updateId, name)} initialName={clientStore.getByID(updateId).name} />
        )}
        {deleteId !== false && (
          <ConfirmDialog
